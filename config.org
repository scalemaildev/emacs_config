...
** Setup
1. Install  [[https://madmalik.github.io/mononoki/][Mononoki]] font (and on Windows, the /All the Icons/ fonts)
...
*** Font
#+begin_src emacs-lisp 
(set-face-attribute 'default nil :font "Mononoki" :height configorg/default-font-size)
(set-face-attribute 'variable-pitch nil :font "Mononoki" :height configorg/default-variable-font-size :weight 'regular)
#+end_src
...
#+begin_src emacs-lisp
(defun configorg/org-babel-tangle-config ()
  (when (string-equal (file-name-directory (buffer-file-name))
                      (expand-file-name user-emacs-directory))
    (let ((org-confirm-babel-evaluate nil))
      (org-babel-tangle))))

(add-hook 'org-mode-hook (lambda () (add-hook 'after-save-hook #'configorg/org-babel-tangle-config)))
#+end_src
...
** Languages
*** Elisp
Run /"lsp-install-server"/ and select elisp for the LSP.

#+begin_src emacs-lisp
(add-hook 'lisp-interaction-mode-hook 'lsp-mode)
#+end_src
...
*** Vue
Make sure Vue/NuxtJS are installed and run /"npm install -g vue-language-server"/ to get the language server.
...
*** Groovy
This will require the LSP from https://github.com/GroovyLanguageServer/groovy-language-server. Make sure to place the .jar file in ~/.cache/emacs/var/lsp/server/ do to our no littering policy. The console output will tell you if it needs to go somewhere else.
